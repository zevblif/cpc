...
跨平台 CI 已上线并在 `main` 分支全绿：

| Job | 平台 | 状态 |
|-----|------|------|
| Test | ubuntu-latest / macos-latest / windows-latest | 通过 |
| Coverage | ubuntu-latest（release 模式 `cargo-llvm-cov` → Codecov） | 通过 |

CT 时序回归测试和高循环次数的统计性表征测试在覆盖率插桩下会被跳过（这些测试仍在无插桩的 Test job 中运行——详见 [`.github/workflows/ci.yml`](.github/workflows/ci.yml)）。
...
本项目为研究原型。主要限制包括：
- **单一参数集**（m=256，编译期常量）
- **覆盖率已通过 Linux CI 测量**（release 模式 `cargo-llvm-cov` → Codecov）。仅覆盖率插桩下跳过三类测试：CT 时序回归测试（插桩引入数据相关开销，会破坏时序断言），以及两个高循环次数的统计性表征测试（`rejection_sampling_statistics`、`prove_completes_within_iteration_budget`，会超出 job timeout）。所有跳过的测试仍在 Test job 中运行。
...
这是一个小型的项目研究，目前**暂不主动接收 pull request**。如果你发现 bug、有问题或想讨论使用场景，请：

- 在 [GitHub Issue](https://github.com/zevblif/cpc/issues) 上提出，或
- 直接发邮件给维护者：**zevblif@163.com**

非常欢迎报告问题与开展讨论——我们只是希望让实现保持一条单一、可审计的提交历史。
